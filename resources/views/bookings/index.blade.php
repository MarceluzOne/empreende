...
@section('content')
{{-- Componente Alpine.js corrigido: vírgulas adicionadas e nomes padronizados --}}
<div x-data="{ 
    openModal: false,
...
class="max-w-6xl mx-auto">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-gray-600 italic">Lista de reservas em Vitória de Santo Antão.</p>
        </div>
        <a href="{{ route('bookings.create') }}" class="bg-blue-900 text-white px-4 py-2 rounded-lg font-bold hover:bg-blue-800 transition shadow-sm">
            ...
            <table class="w-full text-left border-collapse">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-sm font-semibold text-gray-700 text-center">Pessoas</th>
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 text-sm text-center">
                                <div class="flex justify-center space-x-4">
                                    {{-- Visualizar --}}
                                    <button @click="selectedBooking = {{ json_encode($booking) }}; openModal = true" 
    ...
    <div x-show="openModal" 
        ...
        <div @click.away="openModal = false" class="bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden">
            <div class="bg-blue-900 p-5 flex justify-between items-center text-white">
                <h3 class="font-bold text-xl">Detalhes do Registro</h3>
                <button @click="openModal = false"><i class="fas fa-times"></i></button>
            </div>
            <div class="p-8 space-y-4">
                <h4 class="text-xl font-bold text-gray-800" x-text="selectedBooking.responsible_name"></h4>
                <p class="text-gray-600">Pessoas: <span x-text="selectedBooking.guests_count"></span></p>
                <div class="bg-blue-50 p-4 rounded-xl border border-blue-100">
                    <p class="text-blue-900 text-sm italic" x-text="selectedBooking.observation || 'Sem observações.'"></p>
                </div>
            </div>
            <div class="p-4 bg-gray-50 flex justify-end">
                <button @click="openModal = false" class="bg-gray-200 px-6 py-2 rounded-lg font-bold">Fechar</button>
            </div>
        </div>
    </div>
    ...
    <div x-show="openDeleteModal" 
        ...
        <div @click.away="openDeleteModal = false" class="bg-white rounded-2xl shadow-2xl max-w-sm w-full overflow-hidden border-t-4 border-red-600">
            <div class="p-6 text-center">
                ...
                <p class="text-gray-600 text-sm">Deseja excluir o agendamento de:  <span class="font-bold text-red-600" x-text="bookingToDeleteName"></span> ?</p>
            </div>
            <div class="p-4 bg-gray-50 flex space-x-2">
                ...
                <button @click="document.getElementById('delete-form-' + bookingToDelete).submit()" class="w-full bg-red-600 text-white py-2 rounded-lg font-bold shadow-md">Confirmar</button>
            </div>
        </div>
    </div>
</div>
