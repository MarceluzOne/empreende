...
@section('title', 'Editar Palestrante - Empreende Vitória')

@section('content')
<div class="max-w-xl mx-auto">
    <div class="mb-6 flex justify-between items-center">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Editar Palestrante</h2>
            <p class="text-gray-600 italic text-sm">Atualize os dados do palestrante.</p>
        </div>
        <a href="{{ route('speakers.index') }}" class="text-blue-900 font-bold hover:underline text-sm">Voltar</a>
    </div>

    <div class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200">
        <form action="{{ route('speakers.update', $speaker) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-5">
            @csrf
            @method('PUT')

            <div x-data="speakerPhotoCropper('{{ $speaker->photoUrl() }}')" class="flex flex-col items-center gap-3">
                ...
                <button type="button" @click="$refs.rawInput.click()"
                    ...
                    {{ $speaker->photo_path ? 'Alterar foto' : 'Escolher foto' }}
                </button>
            ...
            <div>
                ...
                <textarea name="bio" rows="3"
                    ...
                    placeholder="Breve descrição sobre o palestrante...">{{ old('bio', $speaker->bio) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" name="phone" id="speaker_phone" value="{{ old('phone', $speaker->phone) }}"
                        class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
            ...
            <div>
                ...
                <input type="text" name="cpf" id="speaker_cpf" value="{{ old('cpf', $speaker->formatted_cpf) }}"
            ...
            <div x-data="signatureUpload('{{ $speaker->signatureUrl() }}')">
                ...
                <div class="flex items-center gap-4 flex-wrap">
                    ...
                    <div>
                        ...
                        <button type="button" @click="$refs.sigInput.click()"
                            class="text-sm font-semibold text-blue-600 hover:text-blue-800 transition">{{ $speaker->signature_path ? 'Alterar assinatura' : 'Escolher assinatura' }}</button>
            ...
            {{-- Só administrador define o diretor. Para funcionário o campo nem
                 é renderizado e o SpeakerController preserva o valor atual. --}}
            @if(auth()->user()->isAdmin())
            <div class="border-t pt-6">
                <label class="flex items-start gap-3 cursor-pointer">
                    <input type="checkbox" name="is_director" value="1" @checked(old('is_director', $speaker->is_director))
...
function speakerPhotoCropper(existingUrl) {
    return {
        cropperOpen: false,
