        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 border-b border-gray-100">
                <tr>
                    ...
                    <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Cargo / Objetivo</th>
                    <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider hidden md:table-cell">Área</th>
                    <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider hidden lg:table-cell">Cidade/UF</th>
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50 transition">
                    ...
                    <td class="px-4 py-3">
                        ...
                        <p class="text-xs text-gray-400 mt-0.5">{{ $seeker->formatted_phone ?? $seeker->email ?? '—' }}</p>
                    </td>
                    ...
                    <td class="px-4 py-3 text-gray-600 hidden lg:table-cell text-xs">
                        @if($seeker->city)
                            {{ $seeker->city }}{{ $seeker->state ? ', '.$seeker->state : '' }}
                        @else
    ...
    <div x-show="openModal" x-transition.opacity
        ...
        <div @click.outside="openModal = false"
            ...
            <div class="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <p class="text-xs font-semibold text-gray-400 uppercase mb-1">Cidade / UF</p>
                        <p class="text-gray-700 text-sm"
                            x-text="(selectedSeeker.city ? selectedSeeker.city + (selectedSeeker.state ? ', ' + selectedSeeker.state : '') : '—')"></p>
                    </div>
                    <div>
                        <p class="text-xs font-semibold text-gray-400 uppercase mb-1">Nível</p>
                        <p x-text="selectedSeeker.experience || '—'" class="text-gray-700 text-sm"></p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <template x-if="selectedSeeker.phone">
                            <div class="flex items-center gap-2">
                                <span x-text="selectedSeeker.formatted_phone || selectedSeeker.phone" class="text-gray-700 text-sm"></span>
                                <a :href="'https://wa.me/55' + selectedSeeker.phone.replace(/\D/g, '')"
                                    target="_blank"
                                    class="text-green-500 hover:text-green-600 transition" title="Abrir no WhatsApp">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <p class="text-xs font-semibold text-gray-400 uppercase mb-1">LinkedIn</p>
                        <template x-if="selectedSeeker.linkedin_url">
                            <a :href="selectedSeeker.linkedin_url" target="_blank"
                                class="text-blue-600 text-sm hover:underline break-all" x-text="selectedSeeker.linkedin_url"></a>
                        </template>
                        <template x-if="!selectedSeeker.linkedin_url">
                            <p class="text-gray-400 text-sm">—</p>
                        </template>
                    ...
                    <div>
                        <p class="text-xs font-semibold text-gray-400 uppercase mb-1">GitHub</p>
                        <template x-if="selectedSeeker.github_url">
                            <a :href="selectedSeeker.github_url" target="_blank"
                                class="text-blue-600 text-sm hover:underline break-all" x-text="selectedSeeker.github_url"></a>
                        </template>
                        <template x-if="!selectedSeeker.github_url">
                            <p class="text-gray-400 text-sm">—</p>
                        </template>
                ...
                {{-- Resumo --}}
                <template x-if="selectedSeeker.summary">
                    <div>
                        <p class="text-xs font-semibold text-gray-400 uppercase mb-1">Resumo Profissional</p>
                        <p x-text="selectedSeeker.summary" class="text-gray-700 text-sm leading-relaxed"></p>
                    </div>
                </template>

                {{-- Competências --}}
                <template x-if="selectedSeeker.skills">
                    <div>
                        <p class="text-xs font-semibold text-gray-400 uppercase mb-1">Competências</p>
                        <p x-text="selectedSeeker.skills" class="text-gray-700 text-sm"></p>
                    </div>
                </template>
