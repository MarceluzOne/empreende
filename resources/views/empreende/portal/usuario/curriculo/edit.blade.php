...
<title>Editar Currículo — Empreende Vitória</title>
...
<div class="container">
  ...
  <div class="page-header">
    <div class="page-title">Editar Currículo</div>
    <div class="page-sub">{{ $perfil->name }}</div>
  </div>
  ...
  <form method="POST" action="{{ route('portal.usuario.curriculo.update') }}"
    x-data="{
      cpf: '{{ old('cpf', $perfil->cpf ?? '') }}',
      maskCpf(v){
      ...
      phone: '{{ old('phone', $perfil->formatted_phone ?? '') }}',
      maskPhone(v){
        v=v.replace(/\D/g,'').slice(0,11);
      ...
      experiences: {{ Js::from(old('experiences', $perfil->experiences ?? [])) }},
    ...
    <div class="card">
      ...
      <div class="form-row-3">
        <div class="form-group">
          ...
          <input type="text" name="job_function" value="{{ old('job_function', $perfil->job_function) }}"
            class="form-control {{ $errors->has('job_function') ? 'error' : '' }}">
          @error('job_function')<div class="error-msg">{{ $message }}</div>@enderror
        </div>
        <div class="form-group">
          <label>Cidade</label>
          <input type="text" name="city" value="{{ old('city', $perfil->city) }}" class="form-control">
        </div>
        <div class="form-group">
          <label>Estado (UF)</label>
          <input type="text" name="state" value="{{ old('state', $perfil->state) }}" maxlength="2" class="form-control" style="text-transform:uppercase">
        </div>
      </div>
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <input type="url" name="linkedin_url" value="{{ old('linkedin_url', $perfil->linkedin_url) }}"
            placeholder="https://linkedin.com/in/..." class="form-control">
        </div>
        <div class="form-group">
          <label>GitHub</label>
          <input type="url" name="github_url" value="{{ old('github_url', $perfil->github_url) }}"
        ...
        <div class="form-group">
          ...
          <select name="interest_area" class="form-control {{ $errors->has('interest_area') ? 'error' : '' }}">
            ...
            @foreach($interestAreas as $area)
              <option value="{{ $area }}" {{ old('interest_area', $perfil->interest_area) === $area ? 'selected' : '' }}>{{ $area }}</option>
            @endforeach
          </select>
        ...
        <div class="form-group">
          ...
          <select name="experience" class="form-control">
            ...
            @foreach($experienceLevels as $level)
              <option value="{{ $level }}" {{ old('experience', $perfil->experience) === $level ? 'selected' : '' }}>{{ $level }}</option>
            @endforeach
          </select>
        </div>
      </div>
    ...
    <div class="card">
      ...
      <textarea name="summary" rows="4" class="form-control">{{ old('summary', $perfil->summary) }}</textarea>
    </div>
    ...
    <div class="card">
          ...
          <div class="form-row" style="margin-bottom:10px">
            ...
            <div>
              <input type="text" :name="`experiences[${i}][end]`" x-model="exp.end"
                ...
                :disabled="exp.current" class="form-control">
      ...
      <p x-show="experiences.length===0" class="empty-hint">Nenhuma experiência adicionada.</p>
    </div>
    ...
    <div class="card">
      ...
      <textarea name="skills" rows="3" class="form-control">{{ old('skills', $perfil->skills) }}</textarea>
    </div>
      ...
      <div style="margin-bottom:20px">
        ...
        <template x-for="(lang, i) in languages" :key="i">
          <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">
            <input type="text" :name="`languages[${i}][language]`" x-model="lang.language" placeholder="Idioma" class="form-control" style="flex:1">
