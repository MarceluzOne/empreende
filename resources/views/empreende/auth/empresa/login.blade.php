...
<title>Acesso da Empresa — Empreende Vitória</title>
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .auth-side{width:420px;min-width:380px;background:linear-gradient(145deg,#0f2942 0%,var(--brand-deep) 100%);color:#fff;display:flex;flex-direction:column;justify-content:center;padding:60px 48px;position:relative;overflow:hidden}
  ...
  .auth-side-feature i{width:32px;height:32px;border-radius:8px;background:rgba(255,255,255,.12);display:grid;place-items:center;font-size:13px;flex-shrink:0}
...
<div class="auth-wrap">
  <div class="auth-side">
    <div class="auth-side-icon"><i class="fas fa-building"></i></div>
    <h2>Conecte-se aos <span>melhores talentos.</span></h2>
    <p>Acesse o portal da empresa e encontre os profissionais certos para o seu negócio crescer.</p>
    <div class="auth-side-features">
      <div class="auth-side-feature"><i class="fas fa-plus-circle"></i> Publique vagas de emprego</div>
      <div class="auth-side-feature"><i class="fas fa-users"></i> Visualize candidatos</div>
      <div class="auth-side-feature"><i class="fas fa-chart-line"></i> Gerencie suas publicações</div>
    </div>
  </div>
      ...
      <form action="{{ route('empresa.login.post') }}" method="POST">
        ...
        <div class="form-group">
          ...
          <input type="email" id="email" name="email" class="form-control {{ $errors->has('email') ? 'error' : '' }}"
                 value="{{ old('email') }}" placeholder="empresa@email.com" required>
        </div>
