...
<div x-data="{
    ...
    submitComplete() {
        document.getElementById('form-complete-' + this.completeId).submit();
    }
}">
    ...
    <form method="GET" action="{{ route('attendances.index') }}" class="mb-6">
        ...
        <div class="flex flex-col md:flex-row gap-3">
            ...
            @if(request()->hasAny(['search','status','service_type']))
                <a href="{{ route('attendances.index', ['periodo' => $periodo]) }}" class="px-5 py-2 border rounded-lg text-sm text-gray-600 hover:bg-gray-100 transition flex items-center">
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex justify-center space-x-3">
                                    ...
                                    <form id="form-complete-{{ $attendance->id }}"
                                          action="{{ route('attendances.complete', $attendance->id) }}"
                                          method="POST" class="hidden">
                                        @csrf
                                        @method('PATCH')
                                    </form>
                                    <button type="button"
                                            @click="openComplete('{{ $attendance->id }}')"
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-gray-500 italic">
                                {{ $periodo === 'passadas' ? 'Nenhum atendimento anterior encontrado.' : 'Nenhum atendimento futuro.' }}
                            </td>
                        </tr>
                    @endforelse
                </tbody>
...
<div x-show="openModal" 
    ...
    <div x-show="completeModal" x-cloak
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 w-full max-w-sm mx-4" @click.outside="completeModal = false">
            <div class="flex items-center gap-3 mb-4">
                <div class="w-10 h-10 rounded-full bg-green-100 flex items-center justify-center shrink-0">
                    <i class="fas fa-check-circle text-green-500"></i>
                </div>
                <div>
                    <h2 class="text-base font-semibold text-gray-800">Concluir atendimento</h2>
                    <p class="text-sm text-gray-500">Tem certeza que deseja marcar como concluído?</p>
                </div>
            </div>
            <div class="flex justify-end gap-2 mt-4">
                ...
                <button type="button" @click="submitComplete()"
                    class="px-4 py-2 text-sm rounded-lg bg-green-500 text-white hover:bg-green-600 transition">
                    Concluir
                </button>
            </div>
        </div>
