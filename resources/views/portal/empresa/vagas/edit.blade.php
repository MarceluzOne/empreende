...
<title>Editar Vaga — Empreende Vitória</title>
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  @media(max-width:600px){.form-row,.benefits-grid{grid-template-columns:1fr}.topbar{padding:0 16px}.container{padding:24px 16px}}
</style>
</head>
<body>
...
<div class="container">
  ...
  <div class="page-header">
    <div class="page-title">Editar Vaga</div>
    <div class="page-sub">{{ $vaga->position }} · {{ $vaga->company_name }}</div>
  </div>
  ...
  <form method="POST" action="{{ route('portal.empresa.vagas.update', $vaga) }}"
    x-data="{
      cnpj: '{{ old('cnpj', $vaga->formatted_cnpj) }}',
      remuneration: '{{ old('remuneration', $vaga->remuneration ? number_format((float) $vaga->remuneration, 2, ',', '.') : '') }}',
      maskCnpj(v) {
    ...
    <div class="card">
      ...
      <div class="form-row">
        ...
        <div class="form-group">
          ...
          <input type="text" name="company_name" value="{{ old('company_name', $vaga->company_name) }}"
    ...
    <div class="card">
      ...
      <div class="form-group">
        ...
        <input type="text" name="position" value="{{ old('position', $vaga->position) }}"
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <input type="number" name="quantity" value="{{ old('quantity', $vaga->quantity) }}" min="1"
        ...
        <div class="form-group">
          ...
          <select name="interest_area" class="form-control {{ $errors->has('interest_area') ? 'error' : '' }}">
            ...
            @foreach($interestAreas as $area)
              <option value="{{ $area }}" {{ old('interest_area', $vaga->interest_area) === $area ? 'selected' : '' }}>{{ $area }}</option>
            @endforeach
          </select>
        ...
        <div class="form-group">
          ...
          <select name="min_experience" class="form-control">
            ...
            @foreach($experiences as $exp)
              <option value="{{ $exp }}" {{ old('min_experience', $vaga->min_experience) === $exp ? 'selected' : '' }}>{{ $exp }}</option>
            @endforeach
          </select>
        </div>
      </div>
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <select name="status" class="form-control {{ $errors->has('status') ? 'error' : '' }}">
            <option value="active"  {{ old('status', $vaga->status) === 'active'   ? 'selected' : '' }}>Ativa</option>
            <option value="inactive"{{ old('status', $vaga->status) === 'inactive' ? 'selected' : '' }}>Inativa</option>
            <option value="filled"  {{ old('status', $vaga->status) === 'filled'   ? 'selected' : '' }}>Preenchida</option>
          </select>
      ...
      <div class="form-group">
        ...
        <textarea name="requirements" rows="5" placeholder="Descreva os requisitos..."
          class="form-control {{ $errors->has('requirements') ? 'error' : '' }}">{{ old('requirements', $vaga->requirements) }}</textarea>
        ...
        @foreach($benefits as $benefit)
          <label class="benefit-chip">
            <input type="checkbox" name="benefits[]" value="{{ $benefit }}"
              {{ in_array($benefit, old('benefits', $vaga->benefits ?? [])) ? 'checked' : '' }}>
            {{ $benefit }}
          </label>
        @endforeach
      </div>
